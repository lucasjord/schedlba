...
C
            IF( DAR(KSTA)(1:4) .EQ. 'RDBE' .AND. NCHAN(KS) .GT. 8 ) THEN
               DBE(KS) = 'RDBE_PFB'
               IF( DEBUG ) CALL WLOG( 0, 'Setting to RDBE_PFB' )
            ELSE IF( DAR(KSTA)(1:4) .EQ. 'RDBE' ) THEN
               DBE(KS) = 'RDBE_DDC'
               IF( DEBUG ) CALL WLOG( 0, 'Setting to RDBE_DDC' )
            ELSE IF( DAR(KSTA) .EQ. 'DBBC' ) THEN
               DBE(KS) = 'DBBC_DDC'
               IF( DEBUG ) CALL WLOG( 0, 'Setting to DBBC_DDC' )
            ELSE IF( DAR(KSTA) .EQ. 'WIDAR' ) THEN
               DBE(KS) = 'WIDAR'
               IF( DEBUG ) CALL WLOG( 0, 'Setting to WIDAR' )
            END IF
         END IF
      END DO
      RETURN
