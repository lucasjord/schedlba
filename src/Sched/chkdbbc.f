...
C     Can have up to 4 IFs but actual number depends on DBBCVER
      ...
      DATA        SBWARN / .TRUE. /
      SAVE        SBWARN
      DATA        IFNAM(1), IFNAM(2) / 'A', 'B' /
      DATA        IFNAM(3), IFNAM(4) / 'C', 'D' /
      DATA        IFNAM(5), IFNAM(6) / 'E', 'F' /
      DATA        IFNAM(7), IFNAM(8) / 'G', 'H' /
...
C           sampling.
C           Current version of DBBC (v107) only allows 1-64 MHz channels 
C           and Nyquist sampling.
C
...
C
            DO ICH = 1, NCHAN(KS)
C              IFs are in range A-H, inputs in range 1-4. You may omit the
C              input number, but it's not encouraged.
     ...
     3               IFCHAN(ICH,KS)(1:1) .NE. 'D' .AND.
     4               IFCHAN(ICH,KS)(1:1) .NE. 'E' .AND. 
     5               IFCHAN(ICH,KS)(1:1) .NE. 'F' .AND.
     6               IFCHAN(ICH,KS)(1:1) .NE. 'G' .AND.
     7               IFCHAN(ICH,KS)(1:1) .NE. 'H' )  .OR.
     8             ( IFCHAN(ICH,KS)(2:2) .NE. '1' .AND.
     9               IFCHAN(ICH,KS)(2:2) .NE. '2' .AND.
     *               IFCHAN(ICH,KS)(2:2) .NE. '3' .AND.
     1               IFCHAN(ICH,KS)(2:2) .NE. '4' .AND.
     2               IFCHAN(ICH,KS)(2:2) .NE. ' ' )) THEN
